...
kt0 <- model.matrix(Formula::Formula(data.s0$formula),y.s0,rhs=1)%*%
  data.s0$kl$Estimate
...
w1 <- y.sl.g %>% filter(City==city.grupos[1]) %>% pull(weights) %>% unique()
w2 <- y.sl.g %>% filter(City==city.grupos[2]) %>% pull(weights) %>% unique()
w0 <- y.s0  %>% pull(weights) %>% unique()
...
hist(apply(yt0.matrix, 1, median) )
plot.ts(apply(yt0.matrix, 1, median),ylim=c(0.6,1))
lines(y.s0$RH,col=2)
